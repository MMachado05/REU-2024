...
class BirdseyeLaneDetector:
    ...
    def compute_center(self, image):
        ...
        angle = float(math.degrees(math.atan2(abs(mid - cx), abs(rows - cy))) / 2) * 10 / 180
        
        if cx < mid:
            self.publish_angle(-angle, 0)
        else:
            self.publish_angle(angle, 0)
    ...
    def publish_angle(self, coeff, missed_lanes):
        ...
        if missed_lanes == 2:
            ...
            if (coeff > 1):
                ...
                self.offset_message.angular.x = coeff
