...
def image_callback(ros_image):
    ...
    image = cv_image.copy()
    image = image[int(rows*3.5/5):, int(cols * 1 / 3): int(cols * 2/3)]   

    cv_image = cv_image[rows//2:, int(cols * 1 / 5):]                               # get height and width of image
    ...
    num_yellow_pix = cv2.countNonZero(yellow_mask)
